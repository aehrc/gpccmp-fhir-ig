...
The current scope of writeback includes the QuestionnaireResponse resource only. The [GPCCMP Questionnaire Response](StructureDefinition-GPCCMPQuestionnaireResponse.html) does not derive from an AU Core profile (as one does not exist) and hence has no inherited actor obligations. Therefore, this profile's *Must Support* obligation requirements are defined using GPCCMP actors.
