...
For prepopulation, a [GPCCMP Server](ActorDefinition-GPCCMPServer.html) **SHALL** respond to search requests made by a [GPCCMP Client](ActorDefinition-GPCCMPClient.html) by providing any one of the three variations listed. The GPCCMP Client **SHALL** be able to process all variants. Obligations describing this are included in the profile and can be seen as:
...
- GPCCMP Client [SHALL:process](https://hl7.org/fhir/extensions/CodeSystem-obligation.html#obligation-SHALL.58process) on both `MedicationStatement.medicationCodeableConcept` and `MedicationStatement.medicationReference`.